...
async function syncTiposActividadSeed(db: SQLiteDatabase): Promise<void> {
  ...
  await db.withExclusiveTransactionAsync(async (txn) => {
    for (const tipo of TIPOS_ACTIVIDAD_V1) {
      ...
      await txn.runAsync(
        ...
        [tipo.id, tipo.codigo, tipo.nombre, activo, now],
      );
    }
...
/**
 * Sincroniza `tiposActividad.ts` → SQLite (idempotente + UPSERT).
 * Solo gestiona IDs con prefijo `seed-tipo-`.
 */
export async function ensureTiposActividadSeed(db: SQLiteDatabase): Promise<void> {
  const run = seedQueue.then(() => syncTiposActividadSeed(db));
